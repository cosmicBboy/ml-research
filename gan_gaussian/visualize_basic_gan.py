...
def get_sample_columms(run_samples):
    return [
        c for c in run_samples.columns if c.startswith("sample_")
        and c != "sample_type"]


def plot_axes(
        real_samples, fake_samples, run_metadata,
        ax1, ax2, ax3, ax4, ax5, ax6, scatter_size=2.5):
    ...
    run_metadata.plot(
        ax=ax2, x="epoch", y="real_mu", kind="scatter",
        colormap=blues_cmap, colorbar=False, c=run_metadata["epoch"],
        alpha=0.4, s=scatter_size)
    run_metadata.plot(
        ax=ax2, x="epoch", y="fake_mu", kind="scatter",
        colormap=oranges_cmap, colorbar=False, c=run_metadata["epoch"],
        alpha=0.4, s=scatter_size)

    # plot real and fake sigma
    run_metadata.plot(
        ax=ax3, x="epoch", y="real_sigma", kind="scatter",
        colormap=blues_cmap, colorbar=False, c=run_metadata["epoch"],
        alpha=0.5, s=scatter_size)
    run_metadata.plot(
        ax=ax3, x="epoch", y="fake_sigma", kind="scatter",
        colormap=oranges_cmap, colorbar=False, c=run_metadata["epoch"],
        alpha=0.5, s=scatter_size)

    # plot real and fake skew
    run_metadata.plot(
        ax=ax4, x="epoch", y="real_skew", kind="scatter",
        colormap=blues_cmap, colorbar=False, c=run_metadata["epoch"],
        alpha=0.5, s=scatter_size)
    run_metadata.plot(
        ax=ax4, x="epoch", y="fake_skew", kind="scatter",
        colormap=oranges_cmap, colorbar=False, c=run_metadata["epoch"],
        alpha=0.5, s=scatter_size)

    # plot real and fake kurtosis
    run_metadata.plot(
        ax=ax5, x="epoch", y="real_kurtosis", kind="scatter",
        colormap=blues_cmap, colorbar=False, c=run_metadata["epoch"],
        alpha=0.5, s=scatter_size)
    run_metadata.plot(
        ax=ax5, x="epoch", y="fake_kurtosis", kind="scatter",
        colormap=oranges_cmap, colorbar=False, c=run_metadata["epoch"],
        alpha=0.5, s=scatter_size)
    ...
    run_metadata.plot(
        ax=ax6, x="epoch", y="g_error", color="blue", alpha=0.4)
...
def plot_gan():
    ...
    plot_axes(
        real_samples.iloc[-1], fake_samples.iloc[-1], run_metadata,
    ...
    sns.despine()
    fig.savefig("gan_gaussian_plot.png")
