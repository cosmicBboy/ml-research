...
class TaskEnvironment(object):
    ...
    def _fit(self, ml_framework):
        ...
        if self.enforce_limits and self.ml_framework_fitter.exit_status != 0:
            if self.ml_framework_fitter.exit_status is \
                    pynisher.AnythingException:
                msg = "LIMIT ERROR ml framework fitting error: %s" % \
                    ml_framework
            else:
                msg = (
...
def _ml_framework_fitter(ml_framework, X, y):
    ...
    :returns: a valid ml_framework if it successfully fits a model, None if
        calling the `fit` method leads to fitting error.
    """
